...
class SecurityProvider(Provider):
    ...
    def get_jwt_provider(
        self,
        rabbitmq_settings: RabbitMQSettings,  # Используем для примера, но лучше отдельные settings
    ) -> JwtProvider:
        """Provide PyJwtProvider."""
        # TODO: Вынести JWT секретный ключ в отдельные настройки
        secret_key = "your-secret-key-here"  # Временно, потом из settings
        return PyJwtProvider(secret_key=secret_key)
...
class MessagingProvider(Provider):
    ...
    def get_event_publisher(
        self,
        broker,  # RabbitBroker из InfrastructureProvider
        rabbitmq_settings: RabbitMQSettings,
    ) -> EventPublisher:
